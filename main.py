...
from dependencies import get_current_active_user, get_current_active_superadmin, get_current_active_superadmin_optional, get_current_active_admin_or_superadmin, oauth2_scheme, TokenData
...
async def validate_lectores_preview(
    ...
    try:
        ...
        if tipo_archivo == 'LPR' and 'ID_Lector' in df.columns:
            ...
            lectores_existentes = db.query(models.Lector.ID_Lector).filter(
                models.Lector.ID_Lector.in_(lectores_archivo)
            ).all()
...
async def delete_archivo(id_archivo: int, db: Session = Depends(get_db), current_user: models.Usuario = Depends(get_current_active_user)):
    ...
    try:
        ...
        if archivo_db.Tipo_de_Archivo == "EXTERNO":
            ...
            external_entries = db.query(models.ExternalData).filter(
            ...
            external_data_eliminados = db.query(models.ExternalData).filter(
                models.ExternalData.caso_id == caso_id,
                models.ExternalData.import_date >= fecha_inicio,
                models.ExternalData.import_date <= fecha_fin
            ).delete()
            
            logger.info(f"[Delete] {external_data_eliminados} registros de datos externos eliminados para archivo EXTERNO {archivo_nombre}.")
        ...
        if file_path_to_delete and os.path.isfile(file_path_to_delete):
            try:
                ...
                logger.error(f"[Delete] Error eliminando {file_path_to_delete}: {e}. Continuando... (caso {caso_id}, usuario {current_user.User}).", exc_info=True)
        ...
        if external_data_eliminados > 0:
            logger.info(f"[Delete] Commit realizado. Eliminación completa archivo ID {id_archivo} y {external_data_eliminados} datos externos asociados.")
        else:
...
def read_lecturas(
    skip: int = 0, limit: int = 500000,  # Incrementado a 500K - Sin límites artificiales, optimizado por índices
    ...
    if caso_ids:
        ...
        base_query = base_query.join(models.ArchivoExcel, models.Lectura.ID_Archivo == models.ArchivoExcel.ID_Archivo)\
    ...
    if min_pasos is not None or max_pasos is not None:
        ...
        try:
            ...
            if hora_inicio:
                ...
                pasos_subquery = pasos_subquery.filter(extract('hour', models.Lectura.Fecha_y_Hora) * 100 + extract('minute', models.Lectura.Fecha_y_Hora) >= hora_inicio_dt.hour * 100 + hora_inicio_dt.minute)
            if hora_fin:
                hora_fin_dt = datetime.strptime(hora_fin, "%H:%M")
                pasos_subquery = pasos_subquery.filter(extract('hour', models.Lectura.Fecha_y_Hora) * 100 + extract('minute', models.Lectura.Fecha_y_Hora) <= hora_fin_dt.hour * 100 + hora_fin_dt.minute)
        except ValueError:
        ...
        try:
            ...
            if fecha_hora_combinada:
                ...
                for i in range(0, len(df), PREP_BATCH_SIZE):
                    ...
                    logger.info(f"[Task {task_id}] Procesando lote {i//PREP_BATCH_SIZE + 1}: filas {i} a {min(i+PREP_BATCH_SIZE, len(df))}")
                    
                    # Procesar fechas
                    fechas_procesadas = batch[columna_fecha_hora].apply(lambda x: clean_datetime(x).date() if clean_datetime(x) else None)
                    df.loc[batch.index, 'Fecha'] = fechas_procesadas
                    
                    # Procesar horas
                    horas_procesadas = batch[columna_fecha_hora].apply(lambda x: clean_datetime(x).time() if clean_datetime(x) else None)
        ...
        for i in range(0, len(df), BATCH_SIZE):
            ...
            for index, row in batch_df.iterrows():
                ...
                try:
                    ...
                    logger.info(f"[Task {task_id}] Fila {excel_row_num} - Lectura creada exitosamente: {matricula} - {fecha_hora_final}")
            ...
            if batch_lecturas_obj:
                ...
                logger.info(f"[Task {task_id}] {len(batch_lecturas_obj)} lecturas añadidas al lote. Total acumulado: {lecturas_insertadas_count}")
...
def get_fechas_matricula_gps(caso_id: int, matricula: str, db: Session = Depends(get_db), current_user: models.Usuario = Depends(get_current_active_user)):
    """
...
def get_fechas_caso(caso_id: int, db: Session = Depends(get_db), current_user: models.Usuario = Depends(get_current_active_user)):
    """
...
def reset_sql_password(
    ...
    try:
        ...
        if not success:
                raise HTTPException(
                status_code=500,
                detail="Error al resetear la contraseña SQL"
            )
...
def detectar_vehiculos_lanzadera(
    ...
    vehiculos_acompanantes = defaultdict(lambda: defaultdict(list))  # {matricula: {fecha: [(hora, lector, direccion_temporal), ...]}}
...
def get_caso_performance_warning(caso_id: int, db: Session = Depends(get_db)):
    ...
    try:
        ...
        total_lecturas = db.query(models.Lectura)\
            .join(models.ArchivoExcel)\
            .filter(models.ArchivoExcel.ID_Caso == caso_id)\
            .count()
        
        total_matriculas = db.query(func.count(func.distinct(models.Lectura.Matricula)))\
            .join(models.ArchivoExcel)\
            .filter(models.ArchivoExcel.ID_Caso == caso_id)\
            .scalar()
        ...
        if total_lecturas >= THRESHOLDS["CRITICO"]:
            ...
            recommendations = [
                "Use filtros cuando sea posible para mejor rendimiento"
            ]
