...
class TestAuthentication:
    ...
    def test_login_invalid_credentials(self, client):
        ...
        response = client.post(
            "/api/auth/token", data={"username": "999", "password": "wrong_password"}
        )
    ...
    def test_login_invalid_user_id(self, client):
        ...
        response = client.post(
            "/api/auth/token", data={"username": "invalid_id", "password": "admin123"}
        )
    ...
    def test_check_superadmin_status(self, client, auth_headers_superadmin):
        ...
        response = client.get(
            "/api/auth/check-superadmin", headers=auth_headers_superadmin
        )
    ...
    def test_check_superadmin_status_non_superadmin(
        self, client, auth_headers_admingrupo
    ):
        """Test de verificación de estado superadmin con usuario no superadmin"""
        response = client.get(
            "/api/auth/check-superadmin", headers=auth_headers_admingrupo
        )
