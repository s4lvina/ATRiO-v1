...
class TestCasos:
    ...
    def test_create_caso_superadmin(self, client, auth_headers_superadmin):
        ...
        response = client.post("/casos", json=caso_data, headers=auth_headers_superadmin)
    ...
    def test_create_caso_admingrupo(self, client, auth_headers_admingrupo):
        ...
        caso_data = {"Nombre_del_Caso": "Caso Test Admin", "Año": 2024, "Descripcion": "Caso de prueba creado por admingrupo"}

        response = client.post("/casos", json=caso_data, headers=auth_headers_admingrupo)
    ...
    def test_create_caso_duplicate_name_year(self, client, auth_headers_superadmin, test_caso):
        """Test de creación de caso con nombre y año duplicados"""
        caso_data = {"Nombre_del_Caso": test_caso.Nombre_del_Caso, "Año": test_caso.Año, "ID_Grupo": test_caso.ID_Grupo}

        response = client.post("/casos", json=caso_data, headers=auth_headers_superadmin)

        assert response.status_code == status.HTTP_400_BAD_REQUEST
        assert "Ya existe un caso con el mismo nombre y año" in response.json()["detail"]
    ...
    def test_get_caso_by_id(self, client, auth_headers_admingrupo, test_caso):
        ...
        response = client.get(f"/casos/{test_caso.ID_Caso}", headers=auth_headers_admingrupo)
    ...
    def test_get_caso_by_id_unauthorized(self, client, auth_headers_admingrupo, test_caso):
        ...
        response = client.get(f"/casos/{otro_caso['ID_Caso']}", headers=auth_headers_admingrupo)
    ...
    def test_update_caso_estado(self, client, auth_headers_admingrupo, test_caso):
        ...
        response = client.put(
            f"/casos/{test_caso.ID_Caso}/estado", json={"Estado": nuevo_estado}, headers=auth_headers_admingrupo
        )
    ...
    def test_update_caso_estado_invalid(self, client, auth_headers_admingrupo, test_caso):
        """Test de actualización con estado inválido"""
        response = client.put(
            f"/casos/{test_caso.ID_Caso}/estado", json={"Estado": "EstadoInvalido"}, headers=auth_headers_admingrupo
        )
    ...
    def test_delete_caso_superadmin(self, client, auth_headers_superadmin, test_caso):
        ...
        response = client.delete(f"/casos/{test_caso.ID_Caso}", headers=auth_headers_superadmin)

        assert response.status_code == status.HTTP_204_NO_CONTENT

        # Verificar que el caso fue eliminado
        get_response = client.get(f"/casos/{test_caso.ID_Caso}", headers=auth_headers_superadmin)
        assert get_response.status_code == status.HTTP_404_NOT_FOUND

    def test_delete_caso_admingrupo_own_group(self, client, auth_headers_admingrupo, test_caso):
        """Test de eliminación de caso por admingrupo de su propio grupo"""
        response = client.delete(f"/casos/{test_caso.ID_Caso}", headers=auth_headers_admingrupo)
