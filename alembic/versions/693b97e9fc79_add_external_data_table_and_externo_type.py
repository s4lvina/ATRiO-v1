...
def downgrade() -> None:
    ...
    op.create_index('idx_archivos_caso_america', 'ArchivosExcel', ['ID_Caso', 'ID_Archivo'], unique=False, sqlite_where=sa.text('ID_Caso = 7'))
    op.create_table('ExternalDataSets',
    ...
    op.create_table('ExternalDataEntries',
    ...
    sa.ForeignKeyConstraint(['user_id'], ['usuarios.User'], ),
    sa.PrimaryKeyConstraint('id')
    )
    op.create_index('ix_user_activity_logs_id', 'user_activity_logs', ['id'], unique=False)
    op.create_table('external_vehicle_data',
    sa.Column('id', sa.INTEGER(), nullable=False),
    ...
    sa.ForeignKeyConstraint(['fuente_id'], ['external_data_sources.id'], ),
    sa.PrimaryKeyConstraint('id')
    )
