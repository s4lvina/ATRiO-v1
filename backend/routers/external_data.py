...
async def get_external_data(
    ...
    external_data = db.query(models.ExternalData).filter(
...
async def delete_external_data(
    ...
    external_data = db.query(models.ExternalData).filter(
        models.ExternalData.id == external_data_id
    ).first()
...
async def import_external_data(
    ...
    try:
        ...
        logger.info(f"[External Task {task_id}] External data import '{original_filename}' for caso {caso_id} enqueued for background processing.")
...
async def cross_with_lpr(
    ...
    try:
        ...
        lpr_query = db.query(
            ...
            models.Lector,
            models.Lectura.ID_Lector == models.Lector.ID_Lector
        ).filter(
            and_(
        ...
        for matricula in coincident_matriculas:
            ...
            ).order_by(models.Lectura.Fecha_y_Hora.asc()).first()
            
            if not lpr_reading:
                continue
            
            # Crear exactamente UNA coincidencia por matrícula
            cross_results.append(schemas.ExternalDataCrossResult(
...
async def cross_with_lpr_async(
    ...
    try:
        ...
        # Iniciar procesamiento en segundo plano
        background_tasks.add_task(
            process_cross_data_in_background,
            task_id,
            filters.dict(),
            current_user.User
        )
...
async def get_external_sources(
    ...
    sources = db.query(models.ExternalData.source_name).filter(
        models.ExternalData.caso_id == caso_id
    ).distinct().all()
    ...
    for source in sources:
        ...
        count = db.query(models.ExternalData).filter(
            models.ExternalData.caso_id == caso_id,
...
def process_cross_data_in_background(
    task_id: str,
    filters_dict: dict,
    user_id: int
):
    ...
    try:
        ...
        # Limitar a MAX_RESULTS matrículas si es necesario
        if len(coincident_matriculas) > MAX_RESULTS:
            logger.warning(f"[Cross Task {task_id}] Limitando a {MAX_RESULTS} matrículas de {len(coincident_matriculas)} encontradas")
            coincident_matriculas = set(list(coincident_matriculas)[:MAX_RESULTS])
