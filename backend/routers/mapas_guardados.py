...
def get_mapas_guardados(
    ...
    response_mapas = []
    for mapa in mapas:
        response_mapas.append(MapaGuardadoResponse(
            id=mapa.id,
            nombre=mapa.nombre,
...
def get_mapa_guardado(
    ...
    mapa = db.query(MapaGuardado).filter(
        MapaGuardado.id == mapa_id,
        MapaGuardado.caso_id == caso_id
    ).first()
...
def duplicate_mapa_guardado(
    ...
    mapa_original = db.query(MapaGuardado).filter(
        MapaGuardado.id == mapa_id,
        MapaGuardado.caso_id == caso_id
    ).first()
